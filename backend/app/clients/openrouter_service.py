...
class OpenRouterService:
    ...
    async def _request_with_retry(
        ...
        for attempt in range(self._max_retries + 1):
            ...
            except (OpenRouterRateLimitError, OpenRouterProviderError, OpenRouterTimeoutError, OpenRouterNetworkError) as e:
                last_error = e
    ...
    def _parse_json_content(self, content: str) -> Any:
        ...
        try:
            ...
            raise OpenRouterParseError(
                f"Failed to parse JSON: {e.msg} (excerpt: {excerpt!r})"
            )
    ...
    def _mock_chat_completion(
        ...
        if response_format is not None:
            content = self._generate_mock_structured_response(
        ...
        prompt_tokens = len(user_content.split()) + (len(system_message.split()) if system_message else 0)
