...
class MessageService:
    ...
    async def create_message(
        ...
        try:
            ...
            logger.info(
                f"Created {role.value} message {created_message['id']} " f"for project {project_id}"
            )

            return Message(**created_message)

        except Exception as e:
            logger.error(f"Error creating message for project {project_id}: {e}", exc_info=True)
            raise
    ...
    async def update_csat_rating(
        ...
        try:
            ...
            logger.info(f"Updated CSAT rating to {csat_rating} for message {message_id}")

            return Message(**updated_message)

        except Exception as e:
            logger.error(f"Error updating CSAT rating for message {message_id}: {e}", exc_info=True)
            raise
    ...
    async def get_message(
        ...
        try:
            ...
            if not response.data:
                logger.warning(f"Message {message_id} not found for project {project_id}")
                return None
            ...
            logger.error(
                f"Error getting message {message_id} for project {project_id}: {e}", exc_info=True
            )
            raise
    ...
    async def get_recent_history(
        ...
        try:
            ...
            logger.info(f"Retrieved {len(messages)} recent messages for project {project_id}")

            return messages

        except Exception as e:
            logger.error(
                f"Error getting recent history for project {project_id}: {e}", exc_info=True
            )
            raise
