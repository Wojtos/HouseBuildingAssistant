...
"""
            ...
            logger.info(f"Calling OpenRouter for fact extraction, content length: {len(content_to_analyze)}")
            ...
            logger.info(f"Fact extraction response: {response.content[:200] if response.content else 'empty'}")
            ...
            try:
                ...
                if json_match:
                    ...
                    logger.warning(f"Could not parse JSON from fact extraction response: {response.content[:200]}")
