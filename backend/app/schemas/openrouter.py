...
class ModelParams(BaseModel):
    ...
    max_tokens: Optional[int] = Field(
        default=None, ge=1, le=128000, description="Maximum tokens in response"
    )
    ...
    stop: Optional[Union[list[str], str]] = Field(default=None, description="Stop sequences")
    ...
    def merge(self, overrides: Optional["ModelParams"]) -> "ModelParams":
        ...
        return ModelParams(
            temperature=(
                overrides.temperature if overrides.temperature is not None else self.temperature
            ),
            max_tokens=(
                overrides.max_tokens if overrides.max_tokens is not None else self.max_tokens
            ),
...
class ResponseFormat(BaseModel):
    ...
    @classmethod
    def from_schema(
        cls, name: str, schema: dict[str, Any], strict: bool = True
    ) -> "ResponseFormat":
        """
        ...
        """
        return cls(
            type="json_schema", json_schema=JsonSchemaSpec(name=name, strict=strict, schema=schema)
        )
...
class NormalizedChatResult(BaseModel):
    ...
    raw: Optional[dict[str, Any]] = Field(
        default=None, description="Raw provider response (for debugging, redact before logging)"
    )
