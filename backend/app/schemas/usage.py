...
class UsageSummary(BaseModel):
    ...
    total_tokens: int = Field(description="Total tokens consumed across all APIs")
    total_cost: float = Field(description="Total estimated cost in USD")
    total_requests: int = Field(description="Total number of API requests made")
...
class UsagePeriodItem(BaseModel):
    ...
    period: str = Field(description="Period identifier (e.g., '2024-01-15' for day)")
    tokens: int = Field(description="Tokens consumed in this period")
    cost: float = Field(description="Estimated cost in USD for this period")
    requests: int = Field(description="Number of API requests in this period")
...
class UsageApiItem(BaseModel):
    ...
    tokens: int = Field(description="Total tokens consumed by this API")
    cost: float = Field(description="Total estimated cost for this API in USD")
...
class UsageResponse(BaseModel):
    ...
    summary: UsageSummary = Field(description="Overall usage summary for the period")
    by_period: list[UsagePeriodItem] = Field(description="Usage breakdown by time period")
    by_api: list[UsageApiItem] = Field(description="Usage breakdown by API/service")
...
class ProjectUsageResponse(BaseModel):
    ...
    summary: UsageSummary = Field(description="Usage summary for this project")
    by_api: list[UsageApiItem] = Field(description="Usage breakdown by API/service")
